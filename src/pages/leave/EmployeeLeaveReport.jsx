...
{!isReportLoading && !isReportError && reportLeave?.data && (
  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3 px-6 py-4">
    {reportLeave.data.map((report, i) => {
      ...
      return (
        <motion.div
          ...
          className="group rounded-2xl border border-gray-300 bg-white p-6 shadow-lg dark:border-gray-700 dark:bg-dark-card hover:shadow-xl transition-shadow"
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span
              className={`text-xs font-medium ${color} text-white rounded-full px-3 py-1`}
            >
              {label}
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-sm text-gray-700 dark:text-gray-300">
            <div className="flex items-center gap-2">
              <CalendarCheck size={16} /> {report.totalDays} <span className="hidden sm:inline">Total</span>
            </div>
            <div className="flex items-center gap-2">
              <Clock size={16} /> {report.usedDays} <span className="hidden sm:inline">Used</span>
            </div>
            <div className="flex items-center gap-2">
              <BarChart2 size={16} /> {report.remainingDays} <span className="hidden sm:inline">Remaining</span>
            </div>
          </div>
        </motion.div>
      );
