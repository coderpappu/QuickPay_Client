...
const EmployeeBonusForm = ({ bonusId, onClose }) => {
  ...
  const [createEmployeeBonus] = useCreateEmployeeBonusMutation();

  const [updateEmployeeBonus] = useUpdateEmployeeBonusMutation();
  ...
  const { data: bonusDetails, isLoading: isBonusLoading } =
    useGetEmployeeBonusDetailsQuery(bonusId, {
      skip: !bonusId,
    });

  const initialValues = {
    nameId: bonusDetails?.data?.BonusType?.id || "", // Use ID instead of name
    type: bonusDetails?.data?.type || "",
    value: bonusDetails?.data?.value || "",
  };
...
export default EmployeeBonusForm;
